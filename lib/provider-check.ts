...
export async function checkProviders(repo:Repository,secrets:RuntimeSecrets,reserve:(kind:'search'|'model')=>Promise<void>){
 ...
 const input=['worker-http-v2',secrets.OPENAI_API_KEY??'',secrets.OPENAI_MODEL??'',secrets.BRAVE_SEARCH_API_KEY??'',secrets.BRAVE_STORAGE_RIGHTS??'',secrets.PROVIDER_CHECK_REVISION??''].join('\0');
 ...
 try{
  ...
  else {await reserve('search');const query='site:foerderdatenbank.de Klimaschutz Förderung Kommunen';const u=new URL('https://api.search.brave.com/res/v1/web/search');u.searchParams.set('q',query);u.searchParams.set('count','1');u.searchParams.set('country','DE');const r=await fetch(u,{headers:{'X-Subscription-Token':secrets.BRAVE_SEARCH_API_KEY},redirect:'manual',signal:AbortSignal.timeout(15000)});const d:any=await r.json().catch(()=>({}));report.search={ok:r.ok&&Array.isArray(d.web?.results)&&d.web.results.length>0,http:r.status,query,checkedAt:iso(),resultCount:Array.isArray(d.web?.results)?d.web.results.length:0};if(!report.search.ok)report.search.reason='Suchanbieter liefert keinen erfolgreichen Suchnachweis.';}
